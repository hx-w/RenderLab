...
int main() {
    ...
    printf("\n输出文件为同目录下的table.csv，如果有相同名称的文件将会覆盖内容！\n");
    printf("\n程序会以step为步长在face1中将uv分别在[0.0, 1.0]区间遍历点，并以该点为基准，在face2中以同样步长搜索最近点\n例 步长为0.1，uv方向分别遍历11次，共121种组合");
    printf("\n但由于系统浮点数误差，实际的计算次数有偏差，例如步长为0.05时，uv方向应分别遍历21次，但实际可能只有20次\n");
    printf("\n[输入步长step] ");
    double step = 0.01;
    scanf("%lf", &step);
    cout << endl;
    ...
    cout << endl << endl << "[耗时] " << cost_min << "min " << cost_sec << "s" << endl;
